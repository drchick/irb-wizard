/**
 * Home.jsx — IRBWiz marketing / landing page
 *
 * Sections:
 *  1. HomeNav (sticky header)
 *  2. Hero
 *  3. How It Works (3-step)
 *  4. Who It's For (student / faculty cards)
 *  5. Pricing
 *  6. Footer
 */
...
function Hero() {
  ...
  const handleStart = () => navigate(user ? '/wizard' : '/login');
...
const STEPS = [
  {
    icon: ClipboardList,
    number: '01',
    title: 'Answer 10 guided questions',
    desc: 'Our wizard walks you through every IRB requirement — study design, subjects, risks, consent, and more. No legal jargon, just plain-language prompts.',
  },
  {
    icon: Sparkles,
    number: '02',
    title: 'Get AI-powered review',
    desc: "Claude AI reviews each section for consistency and compliance with 45 CFR 46. You'll see your likely review level (Exempt, Expedited, or Full Board) update in real time.",
  },
  {
    icon: FileDown,
    number: '03',
    title: 'Download your complete package',
    desc: 'Get every document you need: Protocol Description, Consent Form, Recruitment Materials, Parental Permission, HIPAA Authorization, and more — all as formatted Word files.',
  },
];
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
            ...
            <ul className="space-y-2">
              {[
                'Plain-language explanations of federal rules',
                'Flags common student mistakes before submission',
                'Faculty advisor requirements built in',
                'CITI certificate verification',
              ].map(item => (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
            ...
            <ul className="space-y-2">
              {[
                'Full document suite in minutes, not days',
                'AI review flags consistency issues instantly',
                'Standardized templates across your lab',
                'Supports Exempt, Expedited, and Full Board protocols',
              ].map(item => (
                <li key={item} className="flex items-start gap-2 text-sm text-slate-700">
                  <CheckCircle2 size={15} className="text-emerald-500 shrink-0 mt-0.5" />
                  {item}
...
const PLANS = [
  {
    ...
    per: 'per study',
    highlight: false,
    description: 'Perfect for a thesis, dissertation, or one-time research project.',
    features: [
      'Full 10-step protocol wizard',
      'AI-powered section reviews',
      'Complete document package (11 docs)',
      'Unlimited revisions to your protocol',
      'Download as Word (.docx) files',
    ],
    cta: 'Get Started',
    ctaTo: '/login',
    ctaStyle: 'border',
  },
  {
    ...
    price: '$49.99',
    per: '5 studies ($9.99 each)',
    highlight: true,
    badge: 'Most Popular',
    description: 'For faculty, postdocs, or students with multiple protocols to file.',
    features: [
      'Everything in Single Study',
      '5 protocol credits — use anytime',
      'Save 33% vs single-study pricing',
  ...
  },
  {
    ...
    price: '$199',
    per: 'per year · unlimited studies',
    highlight: false,
    description: 'For labs, research centers, and academic departments.',
    features: [
      'Everything in Researcher Pack',
      'Unlimited protocol submissions',
...
function Pricing() {
  ...
  const handleCta = (plan) => {
    if (plan.external) {
      window.location.href = plan.ctaTo;
    } else {
      navigate(user ? '/wizard' : '/login');
    }
  };
          ...
          {PLANS.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl border p-8 flex flex-col ${
                plan.highlight
                  ? 'border-navy-400 bg-navy-900 text-white shadow-xl scale-[1.03]'
            ...
            >
              ...
              <div className="mb-6">
                <h3 className={`font-bold text-lg mb-1 ${plan.highlight ? 'text-white' : 'text-navy-900'}`}>
                  {plan.name}
                </h3>
                <div className="flex items-end gap-1 mb-1">
                  <span className={`text-4xl font-bold ${plan.highlight ? 'text-gold-400' : 'text-navy-800'}`}>
                    {plan.price}
                  </span>
                </div>
                <p className={`text-xs ${plan.highlight ? 'text-slate-400' : 'text-slate-500'}`}>{plan.per}</p>
                <p className={`text-sm mt-3 ${plan.highlight ? 'text-slate-300' : 'text-slate-600'}`}>
                  {plan.description}
                </p>
              </div>
              ...
              <ul className="space-y-2.5 flex-1 mb-8">
                {plan.features.map(f => (
                  <li key={f} className="flex items-start gap-2 text-sm">
                    <CheckCircle2
                      size={15}
                      className={`shrink-0 mt-0.5 ${plan.highlight ? 'text-gold-400' : 'text-emerald-500'}`}
                    />
              ...
              <button
                ...
                className={`w-full py-3 rounded-lg font-semibold text-sm transition-colors ${
                  plan.ctaStyle === 'filled'
                    ...
                    : plan.highlight
                    ? 'border border-slate-500 text-slate-300 hover:border-slate-300 hover:text-white'
